...
fn main() {
    ...
    let xs_c1 = xs[0].clone();
    let t1 = thread::spawn(move || (map_data(&xs_c1)));
    

    let xs_c2 = xs[1].clone();
    let t2 = thread::spawn(move || (map_data(&xs_c2)));
    ...
    for i in 0..num_partitions{
        intermediate_sums_new.push(thread_handles.remove(0).join().unwrap());
    }
